...
type XRepository interface {
	Repository
	XSet(a arena.ByteArena, k, v []byte) error
}
...
func XNew(opts *TableOpts) XRepository {
...
func New(opts *TableOpts) Repository {
	if opts.InMemory {
		return &inMemoryDatabase{
			name:     string(opts.TableName),
			memtable: gmtable.New(),
...
func (db *inMemoryDatabase) XSet(malloc arena.ByteArena, k, v []byte) error {
	return db.memtable.XSet(malloc, k, v)
}
